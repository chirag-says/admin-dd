...
// Component to recenter map — always mounted so it reacts to every position change
...
export default function AdminAddProperty() {
    ...
    // Pre-select builder from URL query param (e.g. ?builderId=xxx)
    ...
    useEffect(() => {
        const loadBuilders = async () => {
            try {
                const data = await builderApi.getAll({ limit: 200, isActive: true });
                setBuilders(data.builders || data.data || []);
            } catch (err) {
    ...
    const [formData, setFormData] = useState({
        builderId: preselectedBuilderId,
        listingType: "Sale", propertyCategory: "Residential", propertyType: "Apartment / Flat",
    ...
    // Fetch metadata via adminApi (auth cookies sent automatically)
    useEffect(() => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Admin-specific: builder ID (required for this endpoint)
            submitData.append("builderId", formData.builderId);

            // POST to admin endpoint — auto-approved, no owner, no reward
            await adminApi.post('/api/properties/admin/add', submitData, {
                headers: { "Content-Type": "multipart/form-data" },
                timeout: 120000,
            });

            toast.success("Property published successfully for builder!");
            navigate('/builder-management');
        } catch (error) {
    ...
    const renderStep1 = () => (
        <div className="space-y-5 sm:space-y-6">
            <div className="text-center">
                <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Builder & Property Type</h2>
                <p className="text-gray-500 mt-1 sm:mt-2 text-sm sm:text-base px-2 hidden sm:block">
                    {preselectedBuilderId ? 'Builder confirmed. Now choose the property category and type.' : 'Select the builder, then choose property category and type.'}
                </p>
            </div>
            ...
            <div>
                ...
                {buildersLoading ? (
                    <div className="flex items-center gap-2 text-gray-500 text-sm py-3">
                        <Loader2 size={16} className="animate-spin" /> Loading builder…
                    </div>
                ) : preselectedBuilderId ? (
                    /* ── Pre-selected from builder row: show locked badge ── */
                    (() => {
                        const b = builders.find(x => x._id === preselectedBuilderId);
                        return b ? (
                            <div className="flex items-center gap-3 px-4 py-3 bg-indigo-50 border-2 border-indigo-200 rounded-xl">
                                <div className="w-9 h-9 rounded-lg bg-indigo-600 flex items-center justify-center flex-shrink-0">
                                    <span className="text-white font-bold text-base">{(b.company || b.name).charAt(0).toUpperCase()}</span>
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="font-bold text-indigo-900 truncate">{b.name}</p>
                                    {b.company && <p className="text-xs text-indigo-600 truncate">{b.company}</p>}
                                </div>
                                <div className="flex items-center gap-1.5 text-indigo-600 text-xs font-semibold bg-indigo-100 px-2.5 py-1 rounded-lg">
                                    <Check size={13} strokeWidth={3} /> Pre-selected
                                </div>
                            </div>
                        ) : (
                            <div className="text-red-500 text-sm py-2">Builder not found. <a href="/builder-management" className="underline">Go back</a></div>
                        );
                    })()
                ) : builders.length === 0 ? (
                    <div className="text-red-500 text-sm py-2">No active builders found. <a href="/builder-management" className="underline">Add a builder first.</a></div>
                ) : (
                    /* ── Standalone access: show editable dropdown ── */
                    <div className="relative">
                        <select
                            name="builderId"
                            value={formData.builderId}
                            onChange={handleChange}
                            className="w-full px-4 py-3 rounded-xl border border-gray-300 bg-white text-gray-900 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none transition-all appearance-none"
                        >
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3 mt-2">
                    {PROPERTY_CATEGORIES[formData.propertyCategory].types.map(type => (
                        <button key={type} onClick={() => handlePropertyTypeChange(type)}
                            ...
                            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl border text-xs sm:text-sm font-medium transition-all ${formData.propertyType === type ? "bg-blue-600 text-white border-blue-600 shadow-md" : "bg-white text-gray-600 border-gray-300 hover:bg-blue-50 hover:border-blue-200"}`}>
                            {type}
                        </button>
                    ))}
                </div>
                    ...
                    <div className="flex flex-wrap gap-2 sm:gap-3 mt-2">
                        {["1 RK", "1 BHK", "2 BHK", "3 BHK", "4 BHK", "5+ BHK", "Studio"].map(bhk => (
                            <button key={bhk} onClick={() => setFormData(p => ({ ...p, bhkType: bhk, bedrooms: bhk === "Studio" ? 0 : (bhk.split(" ")[0]) }))}
                                ...
                                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl border text-xs sm:text-sm font-medium transition-all ${formData.bhkType === bhk ? "bg-blue-600 text-white border-blue-600 shadow-md" : "bg-white text-gray-600 border-gray-300 hover:bg-blue-50 hover:border-blue-200"}`}>
                                {bhk}
                            </button>
                        ))}
                    </div>
    ...
    // Pass an explicit query to bypass the debounced useEffect (used on suggestion select)
    ...
    // Passes lat/lon from JSON through each suggestion so map updates are instant
    ...
    // Uses lat/lon baked into the suggestion from our JSON — zero API calls needed
    const handleSuggestionSelect = (suggestion) => {
        ...
        // ── Instant map update from JSON coords — no API call ──
        if (suggestion.lat && suggestion.lon) {
            const lat = suggestion.lat;
            const lon = suggestion.lon;
            updates.latitude = lat.toFixed(6);
            updates.longitude = lon.toFixed(6);
            setMapPosition([lat, lon]);
            // Cancel any pending debounced geocode — coords already resolved
    ...
    const renderStep2 = () => (
        <div className="space-y-6">
            <div className="text-center">
                ...
                <p className="text-gray-500 mt-2 hidden sm:block">Accurate location increases trust and discoverability.</p>
            </div>
            ...
            <div className={`${cardStyle} relative z-0`}>
                ...
                <div className="rounded-xl overflow-hidden border border-gray-200 relative z-0" style={{ height: "350px" }}>
                    ...
                    >
                        ...
                        />
                        {/* RecenterMap is always mounted so it can react to every position update */}
                        <RecenterMap position={mapPosition} />
                    </MapContainer>
                    {/* Geocoding loading overlay */}
                    {isGeocoding && (
                        <div className="absolute inset-0 bg-white/50 backdrop-blur-[1px] flex items-center justify-center z-[400] rounded-xl pointer-events-none">
                            <div className="flex items-center gap-2 bg-white border border-gray-200 shadow-md px-4 py-2 rounded-full text-sm font-medium text-blue-600">
    ...
    const renderStep3 = () => {
        ...
        return (
            <div className="space-y-6">
                <div className="text-center">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Pricing & Area</h2>
                    <p className="text-gray-500 mt-2 hidden sm:block">Provide accurate area & pricing to attract quality leads.</p>
                </div>
    ...
    const renderStep4 = () => {
        ...
        return (
            <div className="space-y-6">
                <div className="text-center">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Property Details</h2>
                    <p className="text-gray-500 mt-2 hidden sm:block">Add specific features to make your listing stand out.</p>
                </div>
    ...
    const renderStep5 = () => {
        ...
        return (
            <div className="space-y-6">
                <div className="text-center">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Photos & Documents</h2>
                    <p className="text-gray-500 mt-2 hidden sm:block">Click on any category to add photos. All images are optional.</p>
                </div>
                        ...
                        {categories.filter(cat => expandedCategories.includes(cat.key)).map((category) => {
                            ...
                            return (
                                <div key={category.key} className="rounded-2xl p-4 border border-gray-200 bg-white shadow-sm">
                                    ...
                                    <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 sm:p-6 text-center relative hover:border-blue-400 hover:bg-blue-50 transition-all cursor-pointer mb-3 min-h-[80px]">
                                        ...
                                        <div className="text-gray-700 font-medium text-sm sm:text-base">{imageCount >= category.maxImages ? 'Maximum images reached' : 'Tap to upload photos'}</div>
                                        {category.tip && <div className="text-xs text-gray-400 mt-1 hidden sm:block">{category.tip}</div>}
                                    </div>
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                                            {catImages.previews.map((src, idx) => (
                                                <div key={idx} className="relative rounded-lg overflow-hidden aspect-square border border-gray-200">
                                                    ...
                                                    <button onClick={(e) => { e.stopPropagation(); removeCategorizedImage(category.key, idx); }} className="absolute top-1.5 right-1.5 bg-red-600 rounded-full p-1.5 shadow-lg hover:bg-red-700 active:scale-95 transition-transform">
    ...
    const renderStep6 = () => {
        ...
        return (
            <div className="space-y-6">
                <div className="text-center">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Review & Publish</h2>
                    <p className="text-gray-500 mt-2 hidden sm:block">Double-check everything — buyers prefer complete listings.</p>
                </div>
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-gray-900">
                    ...
                    {STEPS.map(step => {
                        ...
                        return (
                            <div key={step.id} className={`group flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 relative overflow-hidden ${isActive ? "bg-white text-red-600 shadow-md" : "text-red-100 hover:bg-white/10 hover:text-white"}`}>
                                ...
                                {isActive && <div className="absolute inset-0 bg-white rounded-2xl transition-all duration-300" />}
                            </div>
                        );
                    })}
                </nav>
            ...
            {/* Mobile Header — fixed right below navbar */}
            <div className="md:hidden bg-red-600 px-3 pt-3 pb-2 fixed top-[56px] left-0 right-0 z-30 shadow-md flex items-center justify-between">
                <div className="flex items-center gap-2 shrink-0">
                    <div className="bg-white/20 p-1.5 rounded-lg"><Home className="text-white" size={16} /></div>
                    <span className="font-bold text-white text-sm sm:text-base">Step {currentStep}<span className="text-white/70 font-normal text-xs ml-1">/ {STEPS.length}</span></span>
                </div>
                <div className="flex gap-1 sm:gap-1.5">
                    {STEPS.map(s => <div key={s.id} className={`h-1.5 flex-1 max-w-8 min-w-4 rounded-full transition-colors ${s.id <= currentStep ? "bg-white" : "bg-red-400/50"}`} />)}
                </div>
            </div>

            <main className="flex-1 p-3 pt-12 sm:p-6 sm:pt-6 md:p-12 max-w-5xl mx-auto w-full flex flex-col h-full">
                <div className="flex-1 pb-20 sm:pb-20 md:pb-0 relative">
                <div className="min-h-0 md:min-h-[520px]">
                    ...
                    <div className="fixed bottom-10 left-3 right-3 md:relative md:bottom-auto md:left-auto md:right-auto bg-white md:bg-transparent border border-gray-200 md:border-t md:border-x-0 md:border-b-0 md:mt-12 md:pt-8 p-3 sm:p-4 md:p-0 flex justify-between items-center z-40 rounded-2xl md:rounded-none shadow-lg md:shadow-none">
                        <button onClick={handlePrev} disabled={currentStep === 1} className={`flex items-center gap-1 sm:gap-2 px-4 sm:px-8 py-2.5 sm:py-3 rounded-xl font-bold transition-all text-sm sm:text-base ${currentStep === 1 ? "text-gray-300 cursor-not-allowed" : "text-gray-600 hover:bg-gray-100"}`}>
                            <ChevronLeft size={16} className="sm:w-[18px] sm:h-[18px]" /> Back
                        </button>

                        <div className="flex items-center gap-2 sm:gap-3">
                            {currentStep < 6 && (
                                <button onClick={handleNext} className="bg-blue-600 text-white px-6 sm:px-10 py-2.5 sm:py-3 rounded-xl font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 flex items-center gap-1.5 sm:gap-2 text-sm sm:text-base">
                                    Next <span className="hidden sm:inline">Step</span> <ArrowRight size={16} className="sm:w-[18px] sm:h-[18px]" />
                                </button>
                            )}
                            {currentStep === 6 && (
                                <button onClick={handleSubmit} disabled={isLoading} className="bg-blue-600 text-white px-6 sm:px-10 py-2.5 sm:py-3 rounded-xl font-bold hover:bg-blue-700 transition-all shadow-lg flex items-center gap-1.5 sm:gap-2 text-sm sm:text-base">
                                    {isLoading ? "Publishing..." : <>Publish <Check size={16} className="sm:w-[18px] sm:h-[18px]" /></>}
                                </button>
                            )}
                        </div>
                    </div>
